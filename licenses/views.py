...
def admin_dashboard(request):
    ...
    onboarding_progress = {}
    try:
        from base.views_onboarding import _get_user_company
        from base.models import Department, EmployeeShift, EmployeeType, Holidays, JobPosition, WorkType
        from leave.models import LeaveType

        ob_company = _get_user_company(request)
        if ob_company:
            onboarding_progress = {
                "departments": Department.objects.filter(company_id=ob_company).count(),
                "positions": JobPosition.objects.filter(company_id=ob_company).count(),
                "shifts": EmployeeShift.objects.filter(company_id=ob_company).count(),
                "employee_types": EmployeeType.objects.filter(company_id=ob_company).count(),
                "work_types": WorkType.objects.filter(company_id=ob_company).count(),
                "holidays": Holidays.objects.filter(company_id=ob_company).count(),
                "leave_types": LeaveType.objects.filter(company_id=ob_company).count(),
            }
            total_configured = sum(1 for v in onboarding_progress.values() if v > 0)
            banner_dismissed = request.session.get("onboarding_banner_dismissed")
            show_onboarding_banner = total_configured < 5 and not banner_dismissed
            onboarding_progress["total_configured"] = total_configured
            onboarding_progress["total_sections"] = 7
    except Exception:
        pass
